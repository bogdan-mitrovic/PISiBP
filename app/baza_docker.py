...
for cat in kategorije:
    cur.execute("INSERT INTO news_category (name) VALUES (%s)", (cat,))
conn.commit()


cur.execute("INSERT INTO auth_user (password, is_superuser, username, email, is_staff, is_active, first_name, last_name, date_joined) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", ("pbkdf2_sha256$260000$6oFRDI3och2qqeMrgKNSi7$1ofwwY49lKQ3D/GVrCwfWwdnKymYA9VOC1qpSOnzZDU=",True ,"pantelija","pantelija@gmail.com",True ,True ,"Nenad","Pantelic",datum))
...
with open('baze.json', 'r') as json_file:
    ...
    for item in data:
        ...
        cur.execute("INSERT INTO news_news (title, content, publish_date, image, views, likes, dislikes, category_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id", (item["title"], item["raw_content"], datum ,item["header_image"], views, likes, dislikes, item["cat"]))
        ...
        for tag_name in item.get("tags", []):
            ...
            cur.execute(
                "SELECT id FROM taggit_tag WHERE name = %s",
                (tag_name,)
            )
            ...
            if existing_tag:
                ...
                cur.execute(
                    "INSERT INTO taggit_tag (name, slug) VALUES (%s, %s)",
                    (tag_name, tag_name)
                )
            ...
            # Insert tagged item
            cur.execute(
                "SELECT 1 FROM taggit_taggeditem WHERE content_type_id = %s AND object_id = %s AND tag_id = (SELECT id FROM taggit_tag WHERE name = %s)",
            ...
            if not existing_tagged_item:
                # Insert tagged item
                cur.execute(
                    "INSERT INTO taggit_taggeditem (content_type_id, object_id, tag_id) VALUES (%s, %s, (SELECT id FROM taggit_tag WHERE name = %s))",
                    (content_type_id, news_id, tag_name)
                )
...
with open('cnn_data.json', 'r') as json_file:
    ...
    for item in data:
        ...
        low = likes + dislikes
        views = random.randint(low, low + 145)
        cur.execute("INSERT INTO news_news (title, content, publish_date, image, views, likes, dislikes, category_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id", (item["title"], item["raw_content"], datum ,item["header_image"], views, likes, dislikes, random.randint(1,6)))
...
with open('huffpost_news_data.json', 'r') as json_file:
    data = json.load(json_file)
    for item in data:
        i = i+1 if i<5 else 1
        datum = datum if i<5 else (datum- timedelta(days=1))
        ...
        views = random.randint(low, low + 145)
        cur.execute(
            "INSERT INTO news_news (title, content, publish_date, image, views, likes, dislikes, category_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id",

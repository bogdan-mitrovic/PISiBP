...
with open('cnn_data.json', 'r') as json_file:
    ...
    for item in data:
        ...
        low = likes + dislikes
        views = random.randint(low, low + 145)
        cur.execute("INSERT INTO news_news (title, content, publish_date, image, views, likes, dislikes, category_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (item["title"], item["raw_content"], datum ,item["header_image"], views, likes, dislikes, i))
...
with open('huffpost_news_data.json', 'r') as json_file:
    ...
    for item in data:
        ...
        cur.execute(
            ...
            (item["headline"], item["raw_description"], datum , item["primary_image"], views, likes, dislikes, i)
        )
...
with open('data_world_example.json', 'r') as json_file:
    ...
    for item in data:
        ...
        cur.execute(
            "INSERT INTO news_news (title, content, publish_date, image, views, likes, dislikes, category_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (item["title"], item["raw_content"], datum, item["header_image"], views, likes, dislikes, i)
        )
